...
from model_utils.get_model import get_alpaca_model_and_tokenizer
...
def main(args):
    ...
    if(args.resume_from_checkpoint):
        model, tokenizer = get_alpaca_model_and_tokenizer("./alpaca_finetune")
        
        # # 加载模型
        # model = AutoModelForCausalLM.from_pretrained("./alpaca_finetune")
        # # 加载分词器
        # tokenizer = AutoTokenizer.from_pretrained("./alpaca_finetune")
    ...
    if save_flag:
        if args.peft:
            ...
            trainer.save_model()
            # 保存模型和 tokenizer
            # model_save_path = './alpaca_finetune'
